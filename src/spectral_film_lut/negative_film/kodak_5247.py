...
class Kodak5247(FilmSpectral):
    def __init__(self, *args, **kwargs):
        ...
        self.type = "negative"
        self.medium = "cine"
        self.alias = "Eastman Color Negative"
        self.comment = "First film with orange mask. Data unreliable."
        ...
        red_log_exposure = (
            xp.array(list(red_curve.keys()), dtype=default_dtype) * log_H_scale
            - log_H_offset
        )
        red_density_curve = (
            xp.array(list(red_curve.values()), dtype=default_dtype) * d_scale
        )
        green_log_exposure = (
            xp.array(list(green_curve.keys()), dtype=default_dtype) * log_H_scale
            - log_H_offset
        )
        green_density_curve = (
            xp.array(list(green_curve.values()), dtype=default_dtype) * d_scale
        )
        blue_log_exposure = (
            xp.array(list(blue_curve.keys()), dtype=default_dtype) * log_H_scale
            - log_H_offset
        )
        blue_density_curve = (
            xp.array(list(blue_curve.values()), dtype=default_dtype) * d_scale
        )
